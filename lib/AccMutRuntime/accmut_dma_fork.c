...
static int recent_set[MAXMUTNUM];
static int recent_num;
static long temp_result[MAXMUTNUM];

typedef struct Eqclass {
    long value;
    int num;
    int mut_id[MMPL * MMPL];
} Eqclass;

static Eqclass eqclass[MMPL * MMPL];
static int eq_num;
...
void __accmut__filter__variant(int from, int to) {
    ...
    if (MUTATION_ID == 0) {
        ...
        /*
        fprintf(stderr,"first filter: num:%d set:\n", recent_num);
        for(i = 0; i < recent_num; ++i)
            fprintf(" %d",recent_set[i]);
        fprintf(stderr, "\n");
         */
        if(recent_num == 0) {
            recent_set[recent_num++] = 0;
...
void __accmut__filter__mutants(int from, int to, int classid) {
    ...
    //fprintf(stderr, "filter: %d %d eqid:%d size:%d\n", MUTATION_ID, MUTATION_ID == 0 ? 0 : forked_active_num, classid, eqclass[classid].num);
}
...
long __accmut__fork__eqclass(int from, int to) {
    ...
    /*
    fprintf(stderr, "recent_set:");
    for(int j = 0; j < recent_num; ++j)
        fprintf(stderr, " %d", recent_set[j]);
    fprintf(stderr, "\n");
    fprintf(stderr, "begin fork: %d %d forked:\n", MUTATION_ID, MUTATION_ID == 0 ? 0 : forked_active_num);
    for(int j = 0; j < forked_active_num; ++j)
        fprintf(stderr, " %d", forked_active_set[j]);
    fprintf(stderr, "\n");
    fprintf(stderr, "eqnum:%d\n eq0:", eq_num);
    for(int j = 0; j < eqclass[0].num; ++j)
        fprintf(stderr, " %d", eqclass[0].mut_id[j]);
    fprintf(stderr, "\n");
    */
    ...
    #if 1
         /*
         fprintf(stderr, "eq%d:", i);
         for(int j = 0; j < eqclass[i].num; ++j)
             fprintf(stderr, " %d", eqclass[i].mut_id[j]);
         fprintf(stderr, "\n");
          */
         fprintf(stderr, "#\n");
    #endif
